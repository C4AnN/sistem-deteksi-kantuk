...
# Streamlit pengaturan antarmuka
st.title("Sistem Deteksi Kantuk dengan YOLOv9")
st.write("Aplikasi ini menggunakan webcam, file video, atau gambar untuk mendeteksi kantuk.")

# Tempatkan frame video dan gambar dalam Streamlit
frame_window = st.image([])

# Variabel untuk mencegah peringatan berulang terlalu cepat
last_alert_time = 0
alert_interval = 5  # Interval dalam detik untuk menghindari peringatan berulang terlalu cepat

# Fungsi untuk menampilkan pesan peringatan di UI
def alert_user():
    st.warning("Deteksi kantuk! Harap waspada.")
        ...
        if len(results[0].boxes) > 0:
            for box in results[0].boxes:
                ...
                if dconf > 0.85 and dclass == 1.0:
                    ...
                    # Cek jika sudah lewat interval waktu untuk memainkan peringatan
                    if current_time - last_alert_time > alert_interval:
                        alert_user()
                        last_alert_time = current_time  # Update waktu terakhir peringatan diberikan
                    break  # Keluar dari loop setelah mendeteksi kantuk
        ...
        if len(results[0].boxes) > 0:
            for box in results[0].boxes:
                ...
                if dconf > 0.85 and dclass == 1.0:
                    ...
                    # Cek jika sudah lewat interval waktu untuk memberikan peringatan
                    if current_time - last_alert_time > alert_interval:
                        alert_user()
                        last_alert_time = current_time  # Update waktu terakhir peringatan diberikan
                    break  # Keluar dari loop setelah mendeteksi kantuk
